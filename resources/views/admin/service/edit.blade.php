...
@section('title', 'Servis Güncelleme')
...
<!--begin::App Content Header-->
        <div class="app-content-header">
          ...
          <div class="container-fluid">
            <!--begin::Row-->
            <div class="row">
              <div class="col-sm-6"><h3 class="mb-0">Servis Güncelleme</h3></div>
              <div class="col-sm-6">
                <ol class="breadcrumb float-sm-end">
                  <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Anasayfa</a></li>
                  <li class="breadcrumb item active" aria-current="page">Servis Yönetimi</li>
                </ol>
              </div>
            </div>
            <!--end::Row-->
                ...
                <div class="card">
                    ...
                    <?php
                        foreach($services as $service){
                            $service_id[$service->lang] = $service->service_id;
                            $title[$service->lang] = $service->title;
                            $description[$service->lang] = $service->description;
                            $seo_url[$service->lang] = $service->seo_url;
                            $seo_title[$service->lang] = $service->seo_title;
                            $seo_description[$service->lang] = $service->seo_description;
                            $seo_keywords[$service->lang] = $service->seo_keywords;
                            $sort[$service->lang] = $service->sort;
                        }
                    ?>
                    <div class="card-body">
                        <form action="{{ route('admin.service.store', $service_id[$language->lang_code]) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            
                            <div class="tab-content" id="myTabContent">
                                @foreach($languages as $language)
                                <input type="hidden" name="service_id" value="{{ $service_id[$language->lang_code] }}">
                            ...
                            <div class="form-group mt-3">
                                ...
                                <a href="{{ route('admin.service.index') }}" class="btn btn-secondary">Geri Dön</a>
                            </div>
                        </form>
                    </div>
                </div>
